-- `development_profiles` (created in 20260819010000_development_profiles.sql) is
-- missing from the live database even though that migration is recorded as
-- applied -- it was dropped outside the migration system at some point after
-- creation. This recreates it identically, which is why get-started.html's
-- "Show me where to look" intake has been failing every submission with a
-- generic "relation does not exist" error (reported by a prospect, Joey
-- Locker, on 2026-09-24/25).
