...
// Collapsed by default so a grid of these reads as a scannable list --
// project, applicant, status, type, a one-line friction summary and
// insight snippet, all drawn straight from fields the expanded view below
// also shows in full (concerns[0] and ai_insight are the same underlying
// data, not a fabricated shorter version). Expanding reveals the original
// 5-section Original Plan -> Friction -> Response -> Outcome -> Insight
// breakdown plus the timeline, unchanged from before.
export default function DevelopmentFrictionCaseCard({ frictionCase }: { frictionCase: DevelopmentFrictionCaseWithSource }) {
  const [expanded, setExpanded] = useState(false);
  const shortFrictionSummary = frictionCase.concerns[0] ?? null;
  ...
  return (
    <div className="rounded-xl border border-[#1c1c1c]/10 bg-white p-4">
      <button type="button" onClick={() => setExpanded((v) => !v)} className="block w-full text-left">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 truncate text-xs text-[#1c1c1c]/50">
              {frictionCase.developer_name ?? "Developer not identified"}
              {frictionCase.project_type ? ` · ${PROJECT_TYPE_LABEL[frictionCase.project_type]}` : ""}
            </p>
          </div>
        ...
        {shortFrictionSummary && <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-[#1c1c1c]/70">{shortFrictionSummary}</p>}

        {frictionCase.ai_insight && (
          <p className="mt-1.5 line-clamp-1 text-xs leading-relaxed text-[#1c1c1c]/50">
            <span className="font-medium text-[#1c1c1c]/70">Insight: </span>
            {frictionCase.ai_insight}
          </p>
        )}

        <p className="mt-2 text-[11px] font-medium text-[#1c1c1c]/35">{expanded ? "Hide details ▲" : "Show details ▼"}</p>
      </button>

      {expanded && (
        <div className="mt-3 space-y-2.5 border-t border-[#1c1c1c]/10 pt-3 text-sm leading-relaxed text-[#1c1c1c]/70">
          ...
          {frictionCase.timeline_events.length > 0 && (
            <div className="border-t border-[#1c1c1c]/10 pt-2">
          ...
          <div className="flex items-center justify-between gap-2 border-t border-[#1c1c1c]/10 pt-2 text-[10px] text-[#1c1c1c]/40">
            <span>
