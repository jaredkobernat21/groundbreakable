...
export default function ShiftDashboardView({
  ...
  return (
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
      <nav className="flex gap-1 overflow-x-auto pb-1 lg:w-44 lg:shrink-0 lg:flex-col lg:gap-0.5 lg:overflow-visible lg:pb-0">
        {TABS.map((tab) => (
          <button
            key={tab.value}
            type="button"
            ...
            className={`shrink-0 rounded-lg px-3 py-2 text-left text-sm font-medium transition lg:w-full ${
              view === tab.value ? "bg-[#1c1c1c] text-white" : "text-[#1c1c1c]/60 hover:bg-[#1c1c1c]/5 hover:text-[#1c1c1c]"
            }`}
          >
            {tab.label}
