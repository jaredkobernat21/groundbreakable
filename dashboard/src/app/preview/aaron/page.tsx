...
// auth gate on purpose -- a share-with-anyone link so Aaron (or Jared) can
// see his real, live personalized dashboard without needing his Supabase
// Auth login (his account exists but has never been invited/sent
// credentials -- see the onboarding migration). Modeled on
// preview/dan-lynch/page.tsx: reads with the service-role client (bypasses
// RLS), but is hardcoded to the one investor row looked up by name below --
// there is no id/slug param, so this route can never expose any other
// investor's data. Read-only: nothing here writes to the DB.
export const metadata: Metadata = {
  robots: { index: false, follow: false },
};

const INVESTOR_FULL_NAME = "Aaron";

export default async function AaronPreviewPage() {
  ...
  if (accountError) {
    console.error("preview/aaron: investor_profiles query failed", accountError);
  }
