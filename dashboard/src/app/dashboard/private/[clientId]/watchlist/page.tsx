...
import { getPrivateClient } from "@/lib/queries/privateClients";
...
const TYPE_HREF: Record<WatchlistItemType, (id: string) => string> = {
  market: () => "/dashboard/private/markets",
  corridor: (id) => `/dashboard/private/corridors/${id}`,
  opportunity: () => "/dashboard",
  shift: () => "/dashboard",
  investment: () => "/dashboard",
};

export default async function WatchlistPage({ params }: { params: { clientId: string } }) {
  ...
  const [items, { data: marketsData }, { data: corridorsData }] = await Promise.all([
    getWatchlist(supabase, client.id),
  ...
  const addMarketAction = addToWatchlist.bind(null, client.id);
  const removeAction = removeFromWatchlist.bind(null, client.id);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-semibold tracking-tight text-[#1c1c1c]">{client.full_name}'s Watchlist</h1>
      </div>

      <section className={cardClass}>
        <h2 className="mb-3 text-sm font-medium uppercase tracking-wide text-[#1c1c1c]/50">Add to Watchlist</h2>
        <div className="grid gap-4 sm:grid-cols-2">
          <form action={addMarketAction} className="flex flex-wrap items-center gap-2">
      ...
      <section className="space-y-2">
        {items.map((item) => {
          const removeItemAction = removeAction.bind(null, item.id);
          return (
        ...
        {items.length === 0 && <p className="text-sm text-[#1c1c1c]/40">Nothing on this client's watchlist yet.</p>}
      </section>
    </div>
  );
}
