import { redirect } from "next/navigation";
import { createClient } from "@/lib/supabase/server";
import type { GrowthArea, Market, PotentialSiteWithSource } from "@/lib/types";
import { POTENTIAL_SITE_TIER_LABEL } from "@/lib/types";
import { createPotentialSite } from "./actions";

export const dynamic = "force-dynamic";

const inputClass =
  "w-full rounded border border-white/10 bg-black/30 px-3 py-2 text-sm text-white outline-none focus:border-white/30";
const labelClass = "mb-1 block text-xs uppercase tracking-wide text-white/40";

export default async function AdminPotentialSitesPage() {
  const supabase = createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();

  const { data: profile } = user
    ? await supabase.from("investor_profiles").select("role").eq("id", user.id).single()
    : { data: null };

  if (profile?.role !== "admin") {
    redirect("/dashboard");
  }

  const [{ data: markets }, { data: growthAreas }, { data: sites }] = await Promise.all([
    supabase.from("markets").select("*").order("name").returns<Market[]>(),
    supabase.from("growth_areas").select("*").order("name").returns<GrowthArea[]>(),
    supabase
      .from("potential_sites")
      .select("*, source:sources(*)")
      .order("created_at", { ascending: false })
      .returns<PotentialSiteWithSource[]>(),
  ]);

  return (
    <div className="space-y-10">
      <div>
        <h1 className="text-2xl font-semibold text-white">Potential Sites — Admin</h1>
        <p className="text-sm text-white/40">
          A parcel or assemblage worth watching for future development. Per the product principle, a single
          acquisition signal (a tax lien, an absentee owner) is never enough on its own -- state the actual
          development thesis in "Why This Site" and cite where it comes from.
        </p>
      </div>

      <section className="rounded-lg border border-white/10 bg-white/5 p-6">
        <h2 className="mb-4 text-sm font-medium uppercase tracking-wide text-white/60">Add a Potential Site</h2>
        <form action={createPotentialSite} className="grid grid-cols-2 gap-4">
          <div>
            <label className={labelClass} htmlFor="market_id">Market</label>
            <select id="market_id" name="market_id" required className={inputClass}>
              {(markets ?? []).map((m) => (
                <option key={m.id} value={m.id}>{m.name}, {m.state}</option>
              ))}
            </select>
          </div>
          <div>
            <label className={labelClass} htmlFor="title">Title</label>
            <input id="title" name="title" required className={inputClass} placeholder="e.g. 1720 Example Rd" />
          </div>

          <div>
            <label className={labelClass} htmlFor="growth_area_id">Growth Area (optional)</label>
            <select id="growth_area_id" name="growth_area_id" defaultValue="" className={inputClass}>
              <option value="">None</option>
              {(growthAreas ?? []).map((g) => (
                <option key={g.id} value={g.id}>{g.name}</option>
              ))}
            </select>
          </div>
          <div>
            <label className={labelClass} htmlFor="tier">Tier</label>
            <select id="tier" name="tier" defaultValue="watch" className={inputClass}>
              <option value="watch">Watch</option>
              <option value="high">High Potential</option>
            </select>
          </div>

          <div>
            <label className={labelClass} htmlFor="address">Address (optional)</label>
            <input id="address" name="address" className={inputClass} />
          </div>
          <div>
            <label className={labelClass} htmlFor="confidence">Confidence</label>
            <select id="confidence" name="confidence" defaultValue="reported" className={inputClass}>
              <option value="verified">Verified against primary source</option>
              <option value="reported">Reported by named source</option>
              <option value="unconfirmed">Unconfirmed / preliminary</option>
            </select>
          </div>

          <div>
            <label className={labelClass} htmlFor="latitude">Latitude</label>
            <input id="latitude" name="latitude" type="number" step="any" required className={inputClass} />
          </div>
          <div>
            <label className={labelClass} htmlFor="longitude">Longitude</label>
            <input id="longitude" name="longitude" type="number" step="any" required className={inputClass} />
          </div>

          <div className="col-span-2">
            <label className={labelClass} htmlFor="development_context">Why This Site (development thesis)</label>
            <textarea
              id="development_context"
              name="development_context"
              rows={4}
              className={inputClass}
              placeholder={"Favorable zoning\nInside Growth Area\nNearby planned development\nUnderutilized property\nArterial frontage\n..."}
            />
          </div>

          <div className="col-span-2 border-t border-white/10 pt-4">
            <h3 className="mb-3 text-xs font-medium uppercase tracking-wide text-white/40">Source</h3>
          </div>

          <div>
            <label className={labelClass} htmlFor="source_agency">Source Agency</label>
            <input id="source_agency" name="source_agency" required className={inputClass} placeholder="e.g. City of Topeka Planning Department" />
          </div>
          <div>
            <label className={labelClass} htmlFor="source_type">Source Type</label>
            <select id="source_type" name="source_type" className={inputClass}>
              <option value="agency_document">Agency Document</option>
              <option value="agency_gis">Agency GIS / Parcel Record</option>
              <option value="press_release">Press Release</option>
              <option value="news">News Article</option>
              <option value="public_record">Public Record</option>
              <option value="other">Other</option>
            </select>
          </div>

          <div>
            <label className={labelClass} htmlFor="source_title">Document / Article Title (optional)</label>
            <input id="source_title" name="source_title" className={inputClass} />
          </div>
          <div>
            <label className={labelClass} htmlFor="source_published_date">Source Published Date (optional)</label>
            <input id="source_published_date" name="source_published_date" type="date" className={inputClass} />
          </div>

          <div className="col-span-2">
            <label className={labelClass} htmlFor="source_url">Source URL</label>
            <input id="source_url" name="source_url" type="url" required className={inputClass} placeholder="https://…" />
          </div>

          <div className="col-span-2">
            <button
              type="submit"
              className="rounded bg-emerald-500 px-4 py-2 text-sm font-medium text-black transition hover:bg-emerald-400"
            >
              Add Potential Site
            </button>
          </div>
        </form>
      </section>

      <section>
        <h2 className="mb-4 text-sm font-medium uppercase tracking-wide text-white/60">
          All Potential Sites ({(sites ?? []).length})
        </h2>
        <div className="space-y-3">
          {(sites ?? []).map((site) => (
            <div key={site.id} className="rounded-lg border border-white/10 bg-white/5 p-4">
              <div className="text-xs uppercase tracking-wide text-white/40">{POTENTIAL_SITE_TIER_LABEL[site.tier]}</div>
              <div className="font-medium text-white">{site.title}</div>
              {site.development_context && <div className="text-sm text-white/50">{site.development_context}</div>}
              {site.source && (
                <a
                  href={site.source.url}
                  target="_blank"
                  rel="noreferrer noopener"
                  className="text-xs text-white/40 underline decoration-white/20 underline-offset-2 hover:text-white"
                >
                  {site.source.agency}
                </a>
              )}
            </div>
          ))}
          {(sites ?? []).length === 0 && <p className="text-sm text-white/40">No Potential Sites entered yet.</p>}
        </div>
      </section>
    </div>
  );
}
