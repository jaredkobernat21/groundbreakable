...
import type { AcquisitionProfile, DevelopmentOpportunity, GrowthArea, Market, MarketIndicator, PrivateClient, Shift } from "./types";
...
// Section 11: assembles the Personalized Private Brief from the same
// data every other tab in the app already reads -- Groundbreakable
// Private's value-add isn't new source data, it's this synthesis layer
// (spec section 14). Everything here is a pure function of
// (client, active acquisition profile, market bundles), so the exact
// same call renders the live brief page and produces what gets
// persisted to private_briefs when the client explicitly saves one.
...
// Which markets this brief should cover: the profile's explicit target
// markets if it names any Groundbreakable-tracked ones, otherwise every
// tracked market (a client with no configured targets yet still gets a
// useful brief instead of an empty one).
export function resolveBriefMarkets(profile: AcquisitionProfile | null, trackedMarkets: Market[]): Market[] {
...
export type PrivateBriefContent = {
...
export function buildPrivateBrief(client: PrivateClient, profile: AcquisitionProfile | null, bundles: MarketBundle[]): PrivateBriefContent {
  ...
  const topSignalSummary = topShifts[0]
    ...
    : "No signals currently on file rise to the top of this client's acquisition profile.";
  ...
  const corridorWatchSummary = topCorridors.length
    ...
    : "No corridors currently match this client's criteria closely enough to lead the watch list.";
  ...
  const cityDecisionsSummary = cityDecisions.length
    ...
    : "No plans/infrastructure decisions logged in the last 90 days across this client's covered markets.";

  const risks: string[] = [];
  if (!profile) risks.push("No acquisition profile is configured yet — matches below default to raw signal strength, not a personalized fit.");
  else if (profile.target_market_ids.length === 0) risks.push("No target markets specified in the acquisition profile — this brief covers every market Groundbreakable currently tracks.");
  if (bundles.every((b) => b.opportunities.length === 0)) risks.push("No development opportunities are currently on file in the markets covered by this brief.");
  const risksSummary = risks.length ? risks.join(" ") : "No material data-coverage risks identified for this brief.";

  const takeParts: string[] = [];
  if (!profile) {
    takeParts.push(`${client.full_name} doesn't have a configured acquisition profile yet — set one up to personalize this brief beyond raw market activity.`);
  } else {
    ...
    if (emergingMarkets[0]) {
      takeParts.push(`${emergingMarkets[0].market.name} currently leads on forward-looking municipal activity across this client's covered markets.`);
    }
  }
